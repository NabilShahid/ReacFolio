jQuery(document).ready(function($) {
  ...
  $(".main-nav-item").on("click", e => {
    ...
    if (fromSection < toSection) {
      let counter = 0;
      for (let i = fromSection + 1; i <= toSection; i++) {
        if (toSection - fromSection == 1) {
          nextSection(e, false);
        } else {
          setTimeout(() => {
      ...
      let counter = 0;
      for (let j = toSection + 1; j <= fromSection; j++) {
        if (fromSection - toSection == 1) prevSection(e, false);
        else {
          setTimeout(() => {
            prevSection(e, j == fromSection ? 700 : 250);
          }, counter * 300);
          counter++;
        }
      }
  ...
  function prevSection(event, duration) {
  ...
  function moveToSectionPrev(event, sectionId) {
    //go to previous section
    typeof event !== "undefined" && event.preventDefault();
  ...
  function nextSection(event, duration) {
    //go to next section
    typeof event !== "undefined" && event.preventDefault();
  ...
  function moveToSectionNext(event, sectionId) {
    //go to next section
    //go to next section
    typeof event !== "undefined" && event.preventDefault();
  ...
  function unbindScroll(section, time) {
    ...
    if (hijacking == "off") {
      $(window).off("scroll", scrollAnimation);
      animationType == "catch"
        ? $("#mainScrollerDiv, html").scrollTop(section.offset().top)
  ...
  function deviceType() {
    ...
    return window
      .getComputedStyle(document.querySelector("#mainScrollerDiv"), "::before")
